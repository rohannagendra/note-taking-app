...
export default function DatabaseTableView({
  ...
  const handleTypeChange = (newType) => {
    ...
    if ((newType === 'select' || newType === 'multiselect') && !menuCol.options) {
      updates.options = [
  ...
  const handleMultiSelectChange = useCallback((rowId, propId, values) => {
    onUpdateRow(rowId, { [propId]: JSON.stringify(values) });
  }, [onUpdateRow]);

  const renderSortIndicator = (propId) => {
    if (sorts && sorts.length > 0) {
      const idx = sorts.findIndex((s) => s.propId === propId);
      if (idx === -1) return null;
      const s = sorts[idx];
      return (
        <span className="sort-indicator">
          {s.dir === 'asc' ? ' \u2191' : ' \u2193'}
          {sorts.length > 1 && <span className="sort-indicator-num">{idx + 1}</span>}
        </span>
      );
    }
        ...
        >
          {renaming ? (
            ...
            <>
              ...
              {(menuCol.type === 'select' || menuCol.type === 'multiselect') && (
...
function MultiSelectCell({ value, options, onChange, onCreateOption }) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState('');
  const ref = useRef(null);
  const inputRef = useRef(null);
  ...
  const getOptionColor = (optValue) => {
    const opt = (options || []).find((o) => o.value === optValue);
    return SELECT_COLORS[opt?.color] || SELECT_COLORS.default;
  };

  return (
    <div className="select-cell-wrapper multiselect-cell-wrapper" ref={ref}>
      <div className="select-cell-trigger multiselect-trigger" onClick={() => setOpen(!open)}>
          ...
          <div className="multiselect-tags">
            {selectedValues.map((v) => (
              <span
                ...
                style={{ background: getOptionColor(v) }}
              >
                {v}
                <span
      ...
      {open && (
        <div className="select-cell-dropdown multiselect-dropdown">
          <input
            ref={inputRef}
            className="select-cell-search"
            type="text"
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && search.trim()) {
                if (filtered.length > 0 && !selectedValues.includes(filtered[0].value)) {
                  toggleValue(filtered[0].value);
                } else if (!exactMatch && onCreateOption) {
                  onCreateOption(search.trim());
                  const newValues = [...selectedValues, search.trim()];
                  onChange(newValues);
                }
                setSearch('');
              }
          ...
          <div className="select-cell-options">
            ...
            {filtered.map((opt) => {
              ...
              return (
                <div
                  key={opt.value}
                  className={'select-cell-option' + (isSelected ? ' active' : '')}
    ...
    case 'multiselect':
      return (
        <MultiSelectCell
          value={value}
          options={col.options || []}
          onChange={(vals) => handlers.handleMultiSelectChange(rowId, col.id, vals)}
          onCreateOption={handlers.onCreateOption ? (optVal) => handlers.onCreateOption(col.id, optVal) : null}
        />
      );
    case 'checkbox':
